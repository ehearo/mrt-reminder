...
outputDir = "src/api-data/Stations/"

service_day_mapping = {"平日": "1,2,3,4,5", "週六": "6", "週日": "7", "假日": "6,7"}
...
def ProcessTimeTable(station_id, all_data):
    ...
    for direction in [0, 1]:
        ...
        for service_day in line_common_data[line_id]["service_day_patterns"]:
            departures = []
            raw_schedule_data = list(
                filter(
                    lambda x: x["Direction"] == direction
                    and x["ServiceDay"]["ServiceTag"] == service_day,
                    station_data,
                )
            )
            ...
            for single_destination_timetable in raw_schedule_data:
                destination_station_id = single_destination_timetable[
                    "DestinationStaionID"
                ]
                if destination_station_id.endswith("A"):
                    continue
                destination_station_name = single_destination_timetable[
                    "DestinationStationName"
                ]["Zh_tw"]
                for time_record in single_destination_timetable["Timetables"]:
                    departures.append(
                        {
                            "Dst": destination_station_name,
                            "Time": time_record["DepartureTime"],
                        }
                    )

            departures.sort(key=lambda k: ConvertToMinute(k["Time"]))
            schedules.append(
                {"Days": service_day_mapping[service_day], "Departures": departures}
            )
